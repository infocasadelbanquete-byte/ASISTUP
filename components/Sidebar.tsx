  ...
  return (
    <>
      <button 
        ...
        className="fixed top-6 left-6 z-50 p-3 bg-blue-700 text-white rounded-2xl md:hidden shadow-2xl active:scale-90 transition-all"
      >
        {isOpen ? (
      ...
      <aside className={`
        fixed inset-y-0 left-0 z-40 w-80 gradient-blue text-white flex flex-col shadow-[15px_0_40px_rgba(0,0,0,0.3)] border-r border-white/5 transition-transform duration-500 md:translate-x-0 md:relative no-print
        ${isOpen ? 'translate-x-0' : '-translate-x-full'}
      `}>
        <div className="p-8 md:p-10 flex-1 overflow-y-auto custom-scroll">
          <div className="flex items-center gap-5 mb-14">
            <div className="w-14 h-14 logo-gradient rounded-2xl flex items-center justify-center shadow-2xl border border-white/20 relative overflow-hidden">
          ...
          {companyName && (
            <div className="bg-white/5 p-4 rounded-2xl mb-10 border border-white/10 backdrop-blur-md">
              <p className="text-[8px] uppercase text-blue-400/80 mb-1 font-black tracking-widest">Organización:</p>
              <p className="text-xs font-black truncate text-white uppercase">{companyName}</p>
            </div>
          )}
        ...
        <div className="p-8 md:p-10 mt-auto">
          <button 
            onClick={onLogout}
            className="w-full flex items-center justify-center gap-4 px-5 py-4 bg-red-500/10 text-red-400 hover:bg-red-500 hover:text-white rounded-2xl transition-all duration-300 font-black border border-red-500/20 uppercase text-[9px] tracking-widest"
          >
            SALIR
          </button>
        </div>
      ...
      {isOpen && <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-30 md:hidden" onClick={() => setIsOpen(false)}></div>}
    </>
  );
};
